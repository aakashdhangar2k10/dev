                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <select class="form-control input-sm" name="type" required>
                      ...
                      <option value="Administrator" <?php echo ($user['type'] == 'Administrator') ? 'selected' : ''; ?>>Admin</option>
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <select class="form-control input-sm" name="branch" required>
                      ...
                      <?php
                      // Fetch branches using PDO
                      $stmt2 = $conn->prepare("SELECT id, short_name FROM branches_tbl");
                      $stmt2->execute();
                      $branches = $stmt2->fetchAll(PDO::FETCH_ASSOC);

                      if ($branches) {
                        foreach ($branches as $row) {
                          $selected = ($user['branch_id'] == $row['id']) ? 'selected' : '';
                          echo '<option value="' . htmlspecialchars($row['id']) . '" ' . $selected . '>'
                            . htmlspecialchars($row['short_name']) . '</option>';
                        }
                      } else {
