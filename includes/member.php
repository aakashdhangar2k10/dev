...
class User
{
	...
	protected static $tbl_name = "accounts";
	function db_fields()
	{
		global $mydb;
	...
	static function AuthenticateUser($usernames = "", $h_upass = "")
	{
		global $mydb;
		$mydb->setQuery("SELECT * FROM `accounts` WHERE `username`='$usernames' AND `password`='$h_upass' LIMIT 1");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur); //get the number of count
		if ($row_count == 1) {
	...
	{
		global $mydb;
		$mydb->setQuery("SELECT * FROM " . self::$tbl_name . " Where oic_id= {$id} LIMIT 1");
